...
import { type Meeting, mapApiMeetingToClient } from '@/entities/meeting';
...
export const myMeetings = createStore<Meeting[]>([], { name: 'myMeetings' })
	...
	.on(triggerMyMeeting, (state, meet) => addMeeting(state, meet))
...
export const endedMeetings = createStore<Meeting[]>([], { name: 'endedMeetings' })
	...
	.on(rejectMeetingRequestFx.doneData, (state, meet) =>
		addMeeting(state, mapApiMeetingToClient({ ...meet, meetingStatus: 'REJECTED' }, 'ENDED')),
	)
